...
class ColumnMappingService {
  readonly fieldOptions = Object.entries(FIELD_LABELS).map(([value, label]) => ({
    value: value as MappableField,
    label,
  }));

  extractColumns(filePath: string): string[] {
    const sheet = this.readFirstSheet(filePath);
    const rows: any[] = XLSX.utils.sheet_to_json(sheet, { defval: '', blankrows: false, header: 1 });
    if (rows.length === 0) throw new ValidationError('Excel file is empty');
    ...
    if (columns.length === 0) throw new ValidationError('Excel file has no column headers');
    return columns;
  }
  ...
  validateMappings(mappings: ColumnMapping[]): void {
    if (!mappings || mappings.length === 0) {
      throw new ValidationError('At least one column mapping is required');
    }
    ...
    if (!hasName && !hasEmail) {
      throw new ValidationError('Map at least one column to Contact Name or Email');
    }
    ...
    for (const m of mappings) {
      ...
      if (seen.has(m.field)) {
        throw new ValidationError(`Field "${FIELD_LABELS[m.field]}" is mapped more than once`);
      }
      seen.add(m.field);
    }
  }
  ...
  parseWithMappings(filePath: string, mappings: ColumnMapping[]): MappedParseResult {
    ...
    const sheet = this.readFirstSheet(filePath);
    const rows: Record<string, any>[] = XLSX.utils.sheet_to_json(sheet, { defval: '', blankrows: false });
    if (rows.length === 0) throw new ValidationError('Excel file has no data rows');

    const mappingByColumn = new Map<string, MappableField>();
    for (const m of mappings) {
    ...
    const orgFields: MappedOrgFields = {};
  ...
  private readFirstSheet(filePath: string) {
    if (!fs.existsSync(filePath)) {
      throw new ValidationError(`Uploaded file not found: ${filePath}`);
    }
  ...
  private scoreColumn(excelColumn: string, usedFields: Set<MappableField>): SuggestedMapping {
    const normalized = this.normalize(excelColumn);
    let best: SuggestedMapping = { excelColumn, field: 'ignore', confidence: 0 };
  ...
  private matchScore(normalized: string, pattern: string): number {
    const p = this.normalize(pattern);
    if (normalized === p) return 100;
    if (normalized.includes(p) || p.includes(normalized)) return 92;
  ...
  private getCellValue(row: Record<string, any>, excelColumn: string): string {
  ...
  private applyContactField(contact: MappedContact, field: MappableField, value: string): void {
    switch (field) {
      case 'contact.name':
        contact.name = value;
        break;
  ...
  private applyOrgField(orgFields: MappedOrgFields, field: MappableField, value: string): void {
    switch (field) {
      case 'org.website':
        if (!orgFields.website) orgFields.website = value;
        break;
      case 'org.industry':
        if (!orgFields.industry) orgFields.industry = value;
        break;
      case 'org.tags':
        if (!orgFields.tags) {
...
export default new ColumnMappingService();
