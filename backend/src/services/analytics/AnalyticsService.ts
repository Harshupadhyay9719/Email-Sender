...
import { Organization, Campaign, EmailLog } from '../../models/index';
import { EmailValidationStatus, CampaignStatus, EmailStatus } from '../../types/index';
...
class AnalyticsService {
  ...
  async getDashboardMetrics(userId: string): Promise<DashboardMetrics> {
    try {
      ...
      const totalOrganizations = await Organization.countDocuments({ createdBy: userObjectId });

      // 2. Contacts counts (total, valid, invalid)
      const contactStats = await Organization.aggregate([
        ...
        { $unwind: '$contacts' },
        {
          $group: {
            ...
            validContacts: {
              $sum: {
                $cond: [{ $eq: ['$contacts.emailValidation.status', EmailValidationStatus.VALID] }, 1, 0],
              },
            },
            invalidContacts: {
              $sum: {
                $cond: [{ $eq: ['$contacts.emailValidation.status', EmailValidationStatus.INVALID] }, 1, 0],
              },
            },
          },
        },
      ...
      const campaigns = await Campaign.find({ createdBy: userObjectId }, { _id: 1 });
      ...
      let contactedOrgs = [];
      if (campaignIds.length > 0) {
        contactedOrgs = await EmailLog.distinct('organizationId', {
      ...
      const companiesWithNoValidContacts = await Organization.countDocuments({
        createdBy: userObjectId,
        'organizationStatus.allContactsInvalid': true,
      });

      // 7. Campaign success rate
      const totalCampaignCount = await Campaign.countDocuments({ createdBy: userObjectId });
      const completedCampaignCount = await Campaign.countDocuments({
        createdBy: userObjectId,
        'config.status': CampaignStatus.COMPLETED,
      });
      const campaignSuccessRate =
        totalCampaignCount > 0 ? parseFloat(((completedCampaignCount / totalCampaignCount) * 100).toFixed(2)) : 0;
      ...
      if (campaignIds.length > 0) {
        dailySendCount = await EmailLog.countDocuments({
          campaignId: { $in: campaignIds },
          status: EmailStatus.SENT,
          'tracking.sentAt': { $gte: oneDayAgo },
        });

        weeklySendCount = await EmailLog.countDocuments({
          campaignId: { $in: campaignIds },
          status: EmailStatus.SENT,
          'tracking.sentAt': { $gte: oneWeekAgo },
        });

        monthlySendCount = await EmailLog.countDocuments({
          campaignId: { $in: campaignIds },
          status: EmailStatus.SENT,
  ...
  async getChartsData(userId: string) {
    try {
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        if (campaignIds.length > 0) {
          sent = await EmailLog.countDocuments({
            campaignId: { $in: campaignIds },
            status: EmailStatus.SENT,
            'tracking.sentAt': { $gte: start, $lte: end },
          });

          opened = await EmailLog.countDocuments({
            campaignId: { $in: campaignIds },
            'tracking.openedAt': { $gte: start, $lte: end },
          });

          failed = await EmailLog.countDocuments({
            campaignId: { $in: campaignIds },
      ...
      const validationDist = await Organization.aggregate([
        { $match: { createdBy: userObjectId } },
        { $unwind: '$contacts' },
        {
          $group: {
      ...
      const validationData = Object.values(EmailValidationStatus).map((status) => {
      ...
      const campaignPerformance = await Campaign.find({ createdBy: userObjectId })
        .sort({ createdAt: -1 })
        .limit(6);
