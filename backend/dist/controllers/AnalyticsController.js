...
class AnalyticsController {
     ...
     */
    static async getReportsMetrics(req, res, next) {
        try {
            if (!req.user) {
                throw new Error('User context missing');
            }
            const userId = req.user.userId;
            const metrics = await AnalyticsService_1.default.getDashboardMetrics(userId);
            const deliveryRate = metrics.emailsSent > 0
                ? parseFloat(((metrics.emailsDelivered / metrics.emailsSent) * 100).toFixed(2))
                : 100;
            const campaignCount = await index_1.Campaign.countDocuments({ createdBy: userId });
            const exportsCount = campaignCount > 0 ? campaignCount + 2 : 1;
            responseHandler_1.ResponseHandler.success(res, 200, 'Reports metrics retrieved successfully', {
                deliveryRate,
            ...
            logger_1.default.error('Error in AnalyticsController.getReportsMetrics:', error);
            next(error);
        }
    }
    /**
    ...
    static async getActivity(req, res, next) {
        try {
            ...
            const chartsData = await AnalyticsService_1.default.getChartsData(req.user.userId);
            responseHandler_1.ResponseHandler.success(res, 200, 'Activity data retrieved successfully', chartsData.dailyActivity);
        }
        catch (error) {
            logger_1.default.error('Error in AnalyticsController.getActivity:', error);
            next(error);
        }
    }
    /**
    ...
    static async getSavedReports(req, res, next) {
        try {
            ...
            responseHandler_1.ResponseHandler.success(res, 200, 'Saved reports retrieved successfully', savedReports);
        }
        catch (error) {
            logger_1.default.error('Error in AnalyticsController.getSavedReports:', error);
            next(error);
        }
    }
    /**
    ...
    static async exportReport(req, res, next) {
        try {
            ...
            const userId = req.user.userId;
            const campaigns = await index_1.Campaign.find({ createdBy: userId }, { _id: 1, campaignName: 1 });
            const campaignIds = campaigns.map((c) => c._id);
            const campaignMap = new Map(campaigns.map((c) => [c._id.toString(), c.campaignName]));
            const emailLogs = await index_1.EmailLog.find({ campaignId: { $in: campaignIds } })
            ...
            if (emailLogs.length === 0) {
                ...
                for (const log of emailLogs) {
                    ...
                    const orgName = log.organizationId?.companyName || 'Unknown';
            ...
            logger_1.default.error('Error in AnalyticsController.exportReport:', error);
            next(error);
        }
    }
}
